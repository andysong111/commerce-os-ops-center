...
test("keyword review UI security forbids direct secret and shell patterns", () => {
  const ui = readFileSync("src/app/keyword-review-queue/page.tsx", "utf8");
  for (const bad of ["child_process", "shell: true", "PowerShell", "powershell", "API_AUTH_KEY", "LOGIN_PASSWORD", "password literal", "secret literal", "https://api.shopling", "localStorage.setItem(\"token"]) assert.equal(ui.includes(bad), false, bad);
});
