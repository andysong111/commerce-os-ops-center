...
test('v0.5.12 batch mode chunks GOODSKEY by 200, opens same-phase windows in parallel, and gates OPTION until every PRICE batch succeeds',async()=>{
...
test('v0.5.12 option-phase failure activates sold-out rollback instead of terminalizing early',async()=>{
...
test('v0.5.12 batch mode caps concurrent Shopling windows at four for a phase',async()=>{
...
test('v0.5.12 explicit result rows retry only failed GOODSKEY at stage 2',async()=>{
...
test('v0.5.12 aggregate failure without row identity narrows failed group 200 -> 20 -> individual',async()=>{
...
test('v0.5.12 third explicit failure becomes relist-required and never enters OPTION',async()=>{
...
test('v0.5.12 unknown completed result is held uncertain and is never auto-resent',async()=>{
  const w=batchWorker(1);
  await w.send('MONTHLY_PRICE_BATCH_START');
  const price=w.current.jobs.find(x=>x.mode==='PRICE'&&x.status==='RUNNING');
  await w.context.commerceOsMonthlyHandleDefinitiveResult(price.id,{
...
test('v0.5.12 sold-out goods that exhaust PRICE retries are rolled back before terminal relist report',async()=>{
  const w=batchWorker(1);
  w.items[0].plan.saleStatusTransition={before:'C',target:'B',restoreAfterTransmission:false};
  w.payload.items=w.items.map(row=>({itemId:row.id,token:row.transmission.token,fingerprint:row.plan.fingerprint}));
  await w.send('MONTHLY_PRICE_BATCH_START');
...
test('v0.5.12 registered-mall readback uses a dedicated command and never reuses price-setting evidence',()=>{
