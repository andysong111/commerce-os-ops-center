...
test("모든 제목의 토큰 multiset이 입력 토큰 multiset과 정확히 같다", () => {
  for (const item of preview(["소매1"]).items) assert.equal(item.title_keyword_integrity_ok, true);
});
...
test("수동 상품명과 수동 검색어가 모두 있어야 수동 후보가 준비된다", async () => {
  const source = await readFile(new URL("../src/components/product-launch-flow/ProductLaunchFlow.tsx", import.meta.url), "utf8");
  assert.match(source, /sourceRowTitle && sourceRowSearch/);
  assert.match(source, /manualTitleCandidatesBySourceRow\[goodsKey\][\s\S]*&&[\s\S]*manualSearchCandidatesBySourceRow\[goodsKey\]/);
});

test("내부 공백이 있는 제목 키워드는 split 무결성 검사를 하지 않고 보존된다", () => {
  const keywords = parseManualMallTitleKeywords("Alpha Pad, Beta Grip, Gamma Case");
  const variant = buildManualMallTitleVariants({ keywords, markets: getMarketsForProductGroup("도매4") })[0];
  assert.equal(variant.title, "Alpha Pad Beta Grip Gamma Case");
  assert.equal(variant.includedKeywordCount, 3);
  assert.equal(variant.keywordIntegrityOk, true);
});

test("상품출시 수동 미리보기는 추천 상품명과 추천 검색어 fallback을 쓰지 않는다", () => {
  const r = row("9901", "도매4");
  const noManual = buildKeywordShoplingPayloadPreview([r], { expandProductGroupMarkets: true });
  assert.equal(noManual.items[0].payload_status, "invalid");
  assert.equal(noManual.items[0].final_title, "");
  assert.equal(noManual.items[0].final_site_srch, "");
});
