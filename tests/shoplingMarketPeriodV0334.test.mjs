...
test("v0.3.34 force-reapplies saved profiles and waits for durable mall-ID selection evidence", async () => {
  ...
  assert.match(source, /function forceSelect/);
  assert.match(source, /dispatchEvent\(new Event/);
...
test("v0.3.34 isolates browser runtime and keeps fail-closed submit boundary", async () => {
  ...
  assert.match(source, /ARM_MESSAGE/);
  assert.match(source, /submit_armed/);
  assert.match(source, /evidence\.selected\.length/);
...
test("v0.3.34 force-applies saved profile on both ID-choice and pre-product screens", async () => {
  const source = await readFile(routePath, "utf8");
  assert.match(source, /register_clicked/);
  assert.match(source, /id_choice_ready/);
  assert.match(source, /id_choice_submitted/);
  assert.match(source, /pre_profile_selected/);
  assert.match(source, /preprod_saved_profile_apply_failed/);
  assert.match(source, /저장검색 재적용/);
  assert.match(source, /change 강제/);
});
