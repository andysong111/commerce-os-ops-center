...
export function buildOneClickIntentV0342(input: Record<string, string>) {
  ...
  agent = replaceRequired(
    ...
    "v0342_auto_agent_guard_anchor_missing",
  );
  agent = replaceRequired(
    agent,
  ...
  for (const marker of [
    ...
    if (!agent.includes(marker)) throw new Error(`v0342_stable_protocol_missing:${marker}`);
  }

  const manifest = JSON.parse(output["manifest.json"]);
  manifest.version = "0.3.42";
  manifest.description = "원클릭 마켓전송이 현재 v0.3.42 queue/intent 원장을 사용하도록 정렬해 A18 저장 프로필과 쇼핑몰 ID 선택을 최신 전송 경로에서 수행합니다. 결과회수·중복방지·최대 3상품/18채널 정책은 유지합니다.";
