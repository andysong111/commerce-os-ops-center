...
function covers(coverage: PurchaseCycleCoverage | undefined, baseline: string, now: number) {
  ...
  return Number.isFinite(start) && Number.isFinite(end) && Number.isFinite(at) &&
    start <= at && end >= at && end <= now + 30_000 && now - end <= MAX_AGE_MS;
}

// A freshly rendered report is not proof of freshly observed sales. Gate on the
// source coverage end, with exact baseline identity, not report.generatedAt.
