...
const REPLACE_RPC = "replace_product_launch_options_atomic";
...
function uniqueOptionId(
  ...
  let candidate = preferred && !used.has(preferred) ? preferred : fallback;
  if (!candidate) candidate = `shopling-option-${index + 1}`;
...
export function mergeLegacySeoShoplingOptionsAtomic(
  ...
  return shoplingOptions.map((shopling, index) => {
    ...
    const matched =
      ...
      {};
    const fallbackId = `shopling-${group.goodsKey}-${text(shopling.optionId) || index + 1}`;
    const optionId = uniqueOptionId(text(matched.id), fallbackId, usedOptionIds, index);
    ...
    return {
      ...
      shoplingOptionSync: {
        source: "shopling_live_grouped_option_sync_atomic",
...
async function patchItem(
  ...
  const itemPayload = {
    ...
    updatedAt: now,
    updatedBy: "이전상품 Shopling 묶음옵션/B코드 원자적 동기화",
  };
  ...
  await readProductLaunchStorageJson(
    `${config.supabaseUrl}/rest/v1/${ITEM_TABLE}?${params.toString()}`,
    {
      ...
      body: JSON.stringify({
        ...
        updated_at: now,
        updated_by: "이전상품 Shopling 묶음옵션/B코드 원자적 동기화",
      }),
      cache: "no-store",
    },
  );
...
async function replaceOptionsAtomically(
  config: ProductLaunchAdminConfig,
  ownerId: string,
  itemId: string,
  options: UnknownRecord[],
  ...
  const ids = rows.map((row) => row.option_id);
  if (new Set(ids).size !== ids.length) {
    throw new Error(`LEGACY_SEO_ATOMIC_OPTION_ID_COLLISION:${itemId}`);
  }

  await readProductLaunchStorageJson(`${config.supabaseUrl}/rest/v1/rpc/${REPLACE_RPC}`, {
    method: "POST",
    headers: {
      ...createSupabaseAdminHeaders(config.secretKey),
...
export async function syncLegacySeoShoplingOptionsAtomic(input: {
  config: ProductLaunchAdminConfig;
  ...
  if (!requested.length) {
    return { changedCount: 0, processedCount: 0, results: [] as SyncResult[] };
  }
  ...
  for (const item of items) {
    ...
    const merged = mergeLegacySeoShoplingOptionsAtomic(current, group);
    ...
    await replaceOptionsAtomically(input.config, input.identity.userId, item.itemId, merged, now);
