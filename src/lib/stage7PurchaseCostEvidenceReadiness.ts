...
export async function loadStage7PurchaseCostEvidenceReadiness(): Promise<Stage7PurchaseCostEvidenceReadiness> {
  ...
  const rows = candidates
    .map((row): Stage7PurchaseCostEvidenceRow => ({
      ...
      hasVerifiedPurchaseCost: verifiedPurchaseCostReady(row, now),
  ...
  const missingVerifiedPurchaseCostCount = coverage.missingCount;
    ...
    priority.state !== "READY"
      ...
      : rows.length === 0
        ...
        : missingVerifiedPurchaseCostCount === 0
          ...
          : `발주 추천 후보 ${rows.length}개 중 ${missingVerifiedPurchaseCostCount}개는 검증원가 근거가 없거나 근거 계약이 불완전해 차단합니다. 상품출시 화면의 수기 원가·과거 캐시·계획 원가는 근거가 연결되기 전까지 자동 승격하지 않습니다.`;
  return {
    generatedAt: new Date().toISOString(),
    state,
    message,
    purchaseCandidateCount: rows.length,
    verifiedPurchaseCostCount: coverage.verifiedCount,
    missingVerifiedPurchaseCostCount,
    confirmedReceiptCount: rows.filter(
      (row) =>
        row.hasVerifiedPurchaseCost &&
        row.purchaseCostTrustSource === "CONFIRMED_RECEIPT",
    ).length,
