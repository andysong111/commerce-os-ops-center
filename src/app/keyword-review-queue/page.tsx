...
export default function KeywordReviewQueuePage() {
  ...
  const dryRunSucceeded = keywordApplyDryRunResult?.summary
    ? String((keywordApplyDryRunResult.summary as Record<string, unknown>).status) === "success"
    : false;
  ...
  function prepareDryRunExecution() {
      ...
      buildKeywordExecutionPreflight(
        ...
        },
        {
          ...
          allowedMallKeys: allowedMallKeys
            ...
            .filter(Boolean),
          maxRows: Number(maxRows) || DEFAULT_KEYWORD_EXECUTION_PREFLIGHT_CONFIG.maxRows,
    ...
    setGuidedActionStatus(
      "적용 점검을 생성했습니다. 아래 dry_run 실행 버튼을 눌러 실제 반영 전 안전 점검을 진행하세요.",
    );
    window.setTimeout(() => {
      document
        .getElementById("keyword-shopling-apply-section")
  ...
  return (
    <>
      ...
      <ProductLaunchWizard
        ...
        guidedActionStatus={guidedActionStatus}
        onPrepareDryRun={prepareDryRunExecution}
      />
...
function KeywordShoplingApplySection({
  ...
  async function run(mode: "dry_run" | "apply") {
    ...
    if (mode === "apply" && !dryRunSucceeded) return;
    const setStatus = mode === "dry_run" ? onDryRunStatusChange : onRealStatusChange;
    const setMeta = metaSetter(mode);
    const setResult = mode === "dry_run" ? onDryRunResultChange : onRealResultChange;
...
function ProductLaunchWizard({
  ...
  dryRunSucceeded: boolean;
  guidedActionStatus: string;
  onPrepareDryRun: () => void;
}) {
  const step4Disabled = !applyPlanReady || counts.previewReadyCount === 0;
  const step5Disabled = !dryRunSucceeded;

  return (
    <section className="my-6 rounded-2xl border border-blue-200 bg-white p-4 shadow-sm sm:p-6">
      <h2 className="text-lg font-bold text-blue-950">순서형 상품 반영 마법사</h2>
      <div className="mt-4 grid gap-3 lg:grid-cols-2">
        <div className="rounded-xl border border-blue-100 bg-blue-50 p-4">
          <p className="text-sm font-bold text-blue-950">Step 4. dry_run 안전 점검 준비</p>
          <p className="mt-2 text-sm text-blue-900">
            적용 계획이 있으면 실행 전 점검을 만들고 아래 dry_run 실행 섹션으로 이동합니다.
          </p>
          <button
            type="button"
            disabled={step4Disabled}
            onClick={onPrepareDryRun}
            className="mt-3 rounded-lg bg-blue-700 px-4 py-2 text-sm font-bold text-white disabled:cursor-not-allowed disabled:bg-slate-300"
          >
            dry_run 실행 준비
          </button>
          {preflightReady ? (
          ...
          {guidedActionStatus ? (
            <p className="mt-3 rounded-lg border border-blue-200 bg-white px-3 py-2 text-sm font-semibold text-blue-900">
              {guidedActionStatus}
            </p>
          ) : null}
        </div>
        <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
          <p className="text-sm font-bold text-slate-950">Step 5. 실제 반영</p>
          ...
          <button
            ...
            className="mt-3 rounded-lg bg-slate-900 px-4 py-2 text-sm font-bold text-white disabled:cursor-not-allowed disabled:bg-slate-300"
          >
            실제 반영 단계로 이동
          </button>
        </div>
