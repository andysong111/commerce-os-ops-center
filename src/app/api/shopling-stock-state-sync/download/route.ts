...
function namespacePriceCoreContent(source: string) {
  ...
  if (!namespaced.includes("  let busy = false;") || !namespaced.includes(originalClaim)) throw new Error("shopling_stock_price_core_claim_adapter_source_mismatch");
  ...
  const retryingClaim = `  async function claim() {
    ...
    try {
      for (let attempt = 0; attempt < 16; attempt += 1) {
        ...
        const response = await chrome.runtime.sendMessage({ type: CLAIM_MESSAGE, role: "A21_POPUP", href: location.href, version: VERSION }).catch(() => null);
        if (response?.ok && response.assignment?.jobId) return await configureAndSubmit(response.assignment);
  ...
  if (!adapted.includes("let claimInFlight = false") || !adapted.includes("attempt < 16") || !adapted.includes('error !== "stock_price_core_not_option_popup_stage"')) throw new Error("shopling_stock_price_core_claim_retry_adapter_missing");
  return adapted;
}
...
export async function GET(request: Request) {
  ...
  if (copiedPriceContent !== canonicalPriceContent || copiedPriceMain !== canonicalPriceMain) throw new Error("shopling_stock_price_core_copy_must_match_canonical_v024");
  ...
  new Function(priceCoreContent); new Function(priceCoreMain);
  ...
  const manifest = JSON.parse(await readFile(path.join(root, "manifest.json"), "utf8")) as {
    ...
    content_scripts: Array<{ js: string[]; matches: string[]; exclude_matches?: string[]; world?: string; all_frames?: boolean }>;
  };
  if (manifest.manifest_version !== 3 || manifest.version !== VERSION) throw new Error("shopling_stock_state_manifest_version_mismatch");
  if (manifest.background.service_worker !== "background-v052.js") throw new Error("shopling_stock_state_background_v052_required");
  for (const permission of ["storage", "tabs", "windows", "scripting", "webNavigation", "alarms"]) if (!manifest.permissions.includes(permission)) throw new Error(`shopling_stock_state_permission_missing:${permission}`);
  for (const host of ["https://a.shopling.co.kr/*", "https://commerce-os-ops-center.vercel.app/*"]) if (!manifest.host_permissions.includes(host)) throw new Error("shopling_stock_state_host_permissions_missing");
  for (const file of [manifest.background.service_worker, manifest.action.default_popup, ...manifest.content_scripts.flatMap((script) => script.js)]) if (!entries[file]) throw new Error(`shopling_stock_state_missing_packaged_file:${file}`);
  ...
  if (!listWorker?.exclude_matches?.some((value) => value.includes("goods_mallMdfy_trsmt.phtml"))) throw new Error("shopling_stock_state_popup_must_be_excluded_from_list_worker");
  if (!popupWorker?.matches?.some((value) => value.includes("goods_mallMdfy_trsmt.phtml"))) throw new Error("shopling_stock_state_price_core_popup_worker_required");
  if (popupMain?.world !== "MAIN" || !popupMain.matches?.some((value) => value.includes("goods_mallMdfy_trsmt.phtml"))) throw new Error("shopling_stock_state_price_core_main_world_required");
  if (!resultObserver?.all_frames) throw new Error("shopling_stock_state_result_observer_all_frames_required");
  ...
  if (new URL(request.url).searchParams.get("verify") === "1") {
    return Response.json({
      ...
      optionBrowserStages: ["A6", "A21_LIST", "A21_POPUP"],
