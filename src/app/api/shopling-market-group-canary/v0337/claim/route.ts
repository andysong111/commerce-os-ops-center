...
export async function POST(request: Request) {
  ...
  const rows = records(record(job.result).rows).filter((row) => row.status === "success");
  ...
  const latest = records(latestResult.data).find((row) => text(record(record(row.payload).seoFinal).source).startsWith("seo-bulk-cloud"));
  ...
  const ledger = records(read.data);
  if (read.error || ledger.length !== 6) return Response.json({ ok: false, error: "safe_claim_ledger_incomplete" }, { status: 409 });
