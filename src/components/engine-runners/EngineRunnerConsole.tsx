  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        {latestRun?.status === "completed" && latestRun.conclusion === "failure" ? (isKeywordRunner ? <div className="mt-4 rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-900"><h3 className="font-bold">환경변수 설정이 필요합니다</h3><p className="mt-1">키워드 엔진 실패 로그에 LOGIN_ID, COMPANY_ID, API_AUTH_KEY, SHOPLING_BASE_URL 또는 필수 환경 변수 누락이 표시되면 GitHub Actions Secrets 설정이 필요합니다.</p><p className="mt-1 text-red-800">설정 후 다시 실행하면 됩니다.</p><Link href="/engine-env-setup" className="mt-3 inline-block rounded-lg bg-red-700 px-3 py-2 text-xs font-semibold text-white">엔진 환경변수 설정하기</Link></div> : <p className="mt-4 rounded-lg bg-red-50 px-3 py-2 text-sm font-semibold text-red-800">이 실행은 실패했습니다. 같은 상품을 다시 실행할 수 있으며, 이전 실패 이력은 결과물 가져오기에 영향을 주지 않습니다.</p>) : null}
