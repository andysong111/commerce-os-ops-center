...
const SAMPLE_1688_TEXT = `https://detail.1688.com/offer/943183302518.html?offerId=943183302518&hotSaleSkuId=5846198593803&spm=a260k.home2025.recommendpart.15
차량용 틈새 수납함 후보 1
price: 9.0
moq: 2
shipping: 10
...
export function SourcingCockpitClient() {
  ...
  async function createCard() {
    ...
    try {
      ...
      const baseCard = buildRecommendationCard({ input: nextInput, settings, candidates: parsed.candidates });
      const nextCard = baseCard.primary ? baseCard : buildLinkReviewCard(nextInput, settings, parsed.candidates);
      setCard(nextCard);
...
function buildLinkReviewCard(input: SourcingInput, settings: SourcingCostSettings, candidates: SourcingCandidate[]): RecommendationCard {
  ...
  const ranked = candidates
    ...
    .map((candidate) => {
      ...
      const score = scoreCandidate(input, candidate, candidates, risk, cost, searchTermsCn);
      return { candidate, risk, cost, score } satisfies RankedCandidate;
    })
    .sort((a, b) => b.score.score - a.score.score);
  const primary = ranked[0] ?? null;
  const backups = ranked.slice(1, 3);
  const hasPrice = Boolean(primary && primary.candidate.unitPriceCny > 0);
  ...
  return {
    ...
    decisionLabel: "보류",
    mode: input.mode,
    modeLabel: "검증제품 저렴하게 따라팔기",
    koreanProductName: input.koreanQuery.trim() || primary?.candidate.titleKr || primary?.candidate.titleCn || "붙여넣은 소싱 후보",
    shortDescription: hasPrice
      ? "후보 링크는 확인됐지만 즉시 주문 전 추가 검토가 필요한 보류 카드입니다."
      : "1688 링크는 확보했지만 단가/MOQ 정보가 부족해 원가 계산 전 보류 상태입니다.",
    searchTermsCn,
    primary,
    backups,
    recommendedOptions: [],
    ...
    estimatedTotalTestCostKrw: hasPrice ? primary?.cost.totalTestCostKrw ?? 0 : 0,
    estimatedUnitCostKrw: hasPrice ? primary?.cost.estimatedUnitCostKrw ?? 0 : 0,
    estimatedMarginRate: hasPrice ? primary?.cost.estimatedMarginRate ?? 0 : 0,
    maxLossKrw: hasPrice ? primary?.cost.maxLossKrw ?? 0 : 0,
    riskLevel: primary?.risk.level === "BLOCKED" ? "BLOCKED" : "HOLD",
    riskNotes: [
      hasPrice ? "주문 전 옵션/재고/포장상태 확인 필요" : "1688 단가가 입력되지 않아 원가/마진 계산은 보류합니다.",
      "1688 상품 페이지에서 단가, MOQ, 중국 내 배송비를 확인한 뒤 다시 붙여넣으면 추천 정확도가 올라갑니다.",
      ...(primary?.risk.reasons ?? []),
    ],
    recommendationReasons: [
      `${candidates.length}개의 실제 1688 링크를 보관했습니다.`,
      hasPrice ? "가격 정보가 일부 확인되어 보류 카드로 저장했습니다." : "링크만 있는 후보는 즉시 주문 가능으로 판정하지 않습니다.",
      "공급처 문의 후 가격/옵션을 보강하면 주문추천 카드 정확도가 올라갑니다.",
    ],
    supplierQuestionsCn: [
      "请确认是否现货。",
      "请确认产品单价、起订量和运费。",
    ...
    costNotice: "이 계산은 정밀 원가가 아니라 테스트 주문 판단용 추정치입니다. 단가가 없으면 원가/마진 계산은 보류됩니다.",
    createdAt: new Date().toISOString(),
  };
}
...
function ResultCard({ card, status, onStatus }: { card: RecommendationCard | null; status?: HumanOrderDecision; onStatus: (status: HumanOrderDecision) => void }) {
  ...
  const hasPrice = Boolean(card.primary && card.primary.candidate.unitPriceCny > 0);
  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-3">
        <Metric label="예상 총 테스트 원가" value={hasPrice ? formatKrw(card.estimatedTotalTestCostKrw) : "단가 필요"} />
        <Metric label="예상 개당 원가" value={hasPrice ? formatKrw(card.estimatedUnitCostKrw) : "계산 보류"} />
        <Metric label="예상 마진율" value={hasPrice ? `${percentFormatter.format(card.estimatedMarginRate * 100)}%` : "계산 보류"} />
      </div>
...
function Badge({ children, tone = "blue" }: { children: ReactNode; tone?: "blue" | "red" | "slate" }) {
