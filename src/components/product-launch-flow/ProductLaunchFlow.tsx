  ...
  return (
    <div className="space-y-6">
      ...
      {keywordSummary.artifact ? <section className="rounded-2xl border border-emerald-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-4 flex flex-wrap gap-3"><button type="button" onClick={openInlineKeywordReview} disabled={!!keywordBusy} className="rounded-lg bg-emerald-700 px-4 py-2 text-sm font-semibold text-white disabled:bg-slate-300">실제 반영 전 최종 확인</button><Link href="/keyword-review-queue?from=product-launch-flow" target="_blank" rel="noopener noreferrer" className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700">개별 키워드 검토 화면에서 열기</Link></div>
      ...
      <FinalDecisionPanel goodsKeyCount={goodsKeys.length} mallCount={priceCounts.okCount || Number(priceActionsResult?.summary?.estimated_mall_update_count ?? 0)} priceStatus={priceCounts.failCount > 0 ? "가격 확인 필요" : isSuccessfulPriceResult(priceActionsResult) ? "정상" : "준비 중"} titleStatus={keywordSummary.artifact ? "반영 준비 완료" : "상품명 일부 누락"} warningCount={priceCounts.failCount} stateLabel={cockpit.primaryAction === "failed" ? "확인 필요" : isSuccessfulPriceResult(priceActionsResult) && keywordSummary.artifact ? "반영 준비 완료" : priceCounts.failCount > 0 ? "가격 확인 필요" : keywordSummary.artifact ? "상품명 일부 누락" : "준비 중"} nextTask={cockpit.nextAction} />
    </div>
  );
}
...
function getPrimaryActionLabel(primaryAction: string, uploadBusy: boolean, priceBusy: boolean, keywordBusy: boolean, currentStage: string) {
  if (uploadBusy) return "준비 중입니다...";
  if (priceBusy) return "준비 중입니다...";
  if (keywordBusy) return "준비 중입니다...";
  if (primaryAction === "upload") return "상품출시 준비 시작";
  if (primaryAction === "price") return "준비 중입니다...";
  if (primaryAction === "keyword") return "AI가 상품명 반영 준비";
  if (primaryAction === "review") return "실제 반영 전 최종 확인";
  if (primaryAction === "failed") return "누락 상품명 자동 보강";
  if (currentStage === "가격설정") return "가격설정 결과 확인 중...";
  if (currentStage === "키워드 결과 검토") return "실제 반영 전 최종 확인";
...
function FinalDecisionPanel({ goodsKeyCount, mallCount, priceStatus, titleStatus, warningCount, stateLabel, nextTask }: { goodsKeyCount: number; mallCount: number; priceStatus: string; titleStatus: string; warningCount: number; stateLabel: "준비 중" | "반영 준비 완료" | "확인 필요" | "실제 반영 완료" | "가격 확인 필요" | "상품명 일부 누락" | string; nextTask: string }) {
  return <section className="rounded-2xl border border-blue-200 bg-white p-6 shadow-sm">
    <p className="text-sm font-bold text-blue-700">복잡한 선택은 시스템이 처리합니다. 실제 샵플링 반영만 사용자가 최종 승인합니다.</p>
    <h2 className="mt-1 text-xl font-black text-slate-950">출시 준비 상태</h2>
    <p className="mt-2 inline-flex rounded-full bg-blue-50 px-3 py-1 text-sm font-black text-blue-800">{stateLabel}</p>
    <dl className="mt-4 grid gap-3 text-sm md:grid-cols-2">
      <ResultRow label="상품 수" value={goodsKeyCount} />
      <ResultRow label="쇼핑몰 반영 수" value={mallCount} />
      <ResultRow label="가격 상태" value={priceStatus} />
      <ResultRow label="상품명 상태" value={titleStatus} />
      <ResultRow label="검색어 경고 수" value={warningCount} />
      <ResultRow label="다음 할 일" value={nextTask} />
    </dl>
    <button type="button" className="mt-5 rounded-xl bg-red-700 px-5 py-3 text-sm font-black text-white shadow-sm hover:bg-red-800">실제 샵플링 반영 실행</button>
  </section>;
}
...
function buildCockpit(state: { hasUploadRequest: boolean; uploadActive: boolean; uploadSuccess: boolean; uploadFailed: boolean; priceActive: boolean; priceSuccess: boolean; priceFailed: boolean; keywordActive: boolean; keywordSuccess: boolean; keywordFailed: boolean }) {
  const steps: CockpitStep[] = [
    ...
    { name: "가격설정", state: state.priceFailed ? "failed" : state.priceActive ? "checking" : state.priceSuccess ? "success" : state.uploadSuccess ? "action" : "waiting", action: state.priceSuccess ? "안전 점검 실행" : state.priceActive ? "가격설정 결과 확인 중..." : "가격설정 시작", message: state.uploadSuccess ? "업로드 성공 후 실행할 수 있습니다." : "업로드 완료 후 활성화됩니다." },
    { name: "키워드/상품명 준비", state: state.keywordFailed ? "failed" : state.keywordActive ? "running" : state.keywordSuccess ? "action" : state.priceSuccess ? "action" : "waiting", action: state.keywordSuccess ? "실제 반영 전 최종 확인" : state.keywordActive ? "키워드 결과 확인 중..." : "안전 점검 실행", message: state.keywordSuccess ? "검토 진행 중 · 승인된 행이 있으면 적용 계획 생성으로 이동" : state.keywordActive ? "키워드 엔진이 실행 중입니다. 결과 파일이 생성되면 자동으로 표시됩니다." : state.keywordFailed ? "키워드 엔진 실행이 실패했습니다." : "dry_run 결과만 준비합니다." },
    { name: "키워드 결과 검토", state: state.keywordSuccess ? "action" : "waiting", action: state.keywordSuccess ? "실제 반영 전 최종 확인" : "artifact 생성 후 열 수 있습니다.", message: state.keywordSuccess ? "검토 진행 중 · 승인된 행이 있으면 적용 계획 생성으로 이동" : "artifact 생성 후 열 수 있습니다." },
