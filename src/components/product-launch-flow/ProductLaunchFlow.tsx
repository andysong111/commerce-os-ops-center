...
import {
  ...
  type ProductLaunchPriceVerificationRow,
...
export function ProductLaunchFlow() {
  ...
  const priceVerificationFailed = hasPriceVerificationIssue(priceActionsResult);
  const cockpit = buildCockpit({
    ...
    priceFailed: hasPriceFailure(priceActionsResult) || priceVerificationFailed,
  ...
  useEffect(() => {
    ...
    if (priceRunning || priceFetching || pricePolling || isSuccessfulPriceResult(priceActionsResult) || hasPriceVerificationIssue(priceActionsResult)) return;
  ...
  useEffect(() => {
    if (!autopilotEnabled) return;
    if (!priceRequestId || !isSuccessfulPriceResult(priceActionsResult) || hasPriceVerificationIssue(priceActionsResult)) return;
  ...
  return (
    <div className="space-y-6">
      ...
      {goodsKeys.length > 0 ? <>
        {priceVerificationFailed ? <p className="rounded-2xl border border-red-200 bg-red-50 p-4 text-sm font-black text-red-800">가격 검증 문제를 먼저 해결해야 다음 단계로 진행할 수 있습니다.</p> : null}
        <KeywordPrepSection blocked={!isSuccessfulPriceResult(priceActionsResult)} rows={uploadRows} goodsKeys={goodsKeys} seedKeyword={keywordSeed} onSeedKeywordChange={setKeywordSeed} preview={keywordPreview} dispatchResult={keywordDispatchResult} runsResult={keywordRunsResult} importMessage={keywordImportMessage} busy={keywordBusy} onPreview={previewKeywordDispatch} onDispatch={dispatchKeywordEngine} onFetchRuns={fetchKeywordRuns} onImport={importKeywordArtifact} />
      </> : null}
      <FinalChecklist priceVerificationFailed={priceVerificationFailed} keywordSucceeded={!!keywordSummary.artifact} />
      </details>
    </div>
  );
}
...
function getPrimaryActionLabel(primaryAction: string, uploadBusy: boolean, priceBusy: boolean, keywordBusy: boolean, currentStage: string) {
  ...
  if (primaryAction === "review") return "키워드 검토 시작";
  if (primaryAction === "failed") return "실패 원인 보기";
  if (currentStage === "가격설정") return "가격설정 결과 확인 중...";
  if (currentStage === "키워드 결과 검토") return "키워드 검토 시작";
...
function PriceSection({ goodsKeyCount, result, actionsResult, requestId, running, fetching, onRun, onFetch }: { goodsKeyCount: number; result: RunResult | null; actionsResult: PriceActionsResult | null; requestId: string; running: boolean; fetching: boolean; onRun: () => void; onFetch: () => void }) {
  ...
  const verificationRows = Array.isArray(summary?.rows) ? summary.rows : [];
  const issueRows = verificationRows.filter((row) => isPriceVerificationIssueStatus(row.verification_status));
  const [showAllVerificationRows, setShowAllVerificationRows] = useState(false);
  const rowsToShow = showAllVerificationRows ? verificationRows : issueRows;
  ...
  const verifiedSuccessCount = Number(summary?.verified_success_count ?? 0);
  ...
  const requiredTargetCount = Number(summary?.estimated_mall_update_count ?? goodsKeyCount * 24);
  ...
  const hasCoverageRisk = notApplied > 0 || blankRisk > 0 || failed > 0 || missingPriceCount > 0 || missingMallRowCount > 0 || mismatchCount > 0 || unverifiedCount > 0;
  const confirmedAll = actionsResult && isSuccessfulPriceResult(actionsResult);
  return <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><h2 className="text-lg font-bold text-slate-950">Step 2. 가격설정</h2><p className="mt-3 text-sm text-slate-700">대상 goods_key 수: <strong>{goodsKeyCount}</strong></p><p className="mt-1 text-sm text-slate-700">예상 쇼핑몰 가격설정 대상 수 = goods_key count × 24: <strong>{goodsKeyCount * 24}</strong></p><button type="button" onClick={onRun} disabled={running} className="mt-5 rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white disabled:bg-slate-400">{running ? "실행 요청 중..." : "가격설정 실행"}</button><button type="button" onClick={onFetch} disabled={fetching} className="ml-3 mt-5 rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white disabled:bg-slate-400">{fetching ? "가져오는 중..." : "가격설정 결과 가져오기"}</button><StatusBlock result={result} requestId={requestId} />{actionsResult ? <div className={hasCoverageRisk ? "mt-4 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-950" : "mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900"}><h3 className="font-black">가격 검증 결과</h3><p className="mt-2 font-bold">{confirmedAll ? "모든 필수 쇼핑몰 가격 반영을 확인했습니다." : "가격이 비어 있거나 확인되지 않은 쇼핑몰이 있습니다."}</p>{hasCoverageRisk ? <><p className="mt-2 font-semibold">가격 검증 문제를 먼저 해결해야 다음 단계로 진행할 수 있습니다.</p><p className="mt-1">가격이 비어 있을 수 있는 쇼핑몰이 있습니다.</p></> : null}<dl className="mt-3 grid gap-2 md:grid-cols-3"><ResultRow label="필수 가격 대상" value={String(requiredTargetCount)} /><ResultRow label="검증 성공" value={String(verifiedSuccessCount)} /><ResultRow label="가격 비어 있음" value={String(missingPriceCount)} /><ResultRow label="쇼핑몰 행 없음" value={String(missingMallRowCount)} /><ResultRow label="불일치" value={String(mismatchCount)} /><ResultRow label="검증 불가" value={String(unverifiedCount)} /></dl>{hasCoverageRisk ? <><p className="mt-2">영향 쇼핑몰: {affectedMalls || "확인 필요"}</p><p>영향 goods_key 수: {Array.isArray(summary?.affected_goods_keys) ? summary.affected_goods_keys.length : String(summary?.goods_key_count ?? goodsKeyCount)}</p><div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={onRun} className="rounded-lg bg-red-700 px-3 py-2 text-xs font-bold text-white">가격설정 재실행</button><GithubActionsShortcutButton href={actionsResult?.runUrl} /><details className="rounded-lg border border-red-300 px-3 py-2 text-xs font-bold text-red-800"><summary className="cursor-pointer">상세 가격 검증 결과 보기</summary><p className="mt-2 font-normal">문제 행과 GitHub Actions 로그를 함께 확인하세요.</p></details></div></> : null}</div> : null}{actionsResult ? <dl className="mt-4 grid gap-3 text-sm"><ResultRow label="status" value={String(summary?.status ?? actionsResult.status ?? "-")} /><ResultRow label="exit_code" value={String(summary?.exit_code ?? "-")} /><ResultRow label="goods_key_count" value={String(summary?.goods_key_count ?? "-")} /><ResultRow label="estimated_mall_update_count" value={String(summary?.estimated_mall_update_count ?? "-")} /><ResultRow label="policy_override_count" value={String(summary?.policy_override_count ?? 0)} /><ResultRow label="성공 수" value={String(summary?.ok_count ?? "-")} /><ResultRow label="실패 수" value={String(summary?.fail_count ?? "-")} /></dl> : null}{verificationRows.length > 0 ? <PriceVerificationTable rows={rowsToShow} showAll={showAllVerificationRows} onShowAllChange={setShowAllVerificationRows} /> : null}<ErrorsTable errors={errors} /></section>;
}

function KeywordPrepSection({ blocked, rows, goodsKeys, seedKeyword, onSeedKeywordChange, preview, dispatchResult, runsResult, importMessage, busy, onPreview, onDispatch, onFetchRuns, onImport }: { blocked: boolean; rows: ProductLaunchUploadRow[]; goodsKeys: string[]; seedKeyword: string; onSeedKeywordChange: (value: string) => void; preview: unknown; dispatchResult: KeywordDispatchResult | null; runsResult: KeywordRunsResult | null; importMessage: string; busy: string; onPreview: () => void; onDispatch: () => void; onFetchRuns: () => void; onImport: (run: KeywordRun, artifact: KeywordArtifact) => void }) {
  const latestRunWithArtifact = runsResult?.runs?.find((run) => run.artifacts?.some((artifact) => artifact.expected && !artifact.expired));
  return <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><h2 className="text-lg font-bold text-slate-950">Step 3. 상품명/키워드 실행 및 검토</h2><p className="mt-3 text-sm text-slate-700">현재 MVP에서는 상품명/키워드를 6개 상품코드에 동일하게 적용하는 기준으로 준비합니다.</p><p className="mt-1 text-sm text-slate-700">키워드 엔진은 dry_run으로만 실행되며, 결과는 키워드 결과 검토 화면에서 사람이 확인합니다.</p><p className="mt-2 rounded-lg bg-red-50 p-3 text-sm font-bold text-red-700">키워드/상품명 결과는 샵플링에 자동 반영되지 않습니다. 검토 화면에서 확인 후 별도 승인해야 합니다.</p><p className="mt-3 text-sm text-slate-700">대상 goods_key 수: <strong>{goodsKeys.length}</strong></p><p className="mt-1 break-all font-mono text-xs text-slate-700">goods_key CSV preview: {goodsKeys.join(",")}</p><label className="mt-4 block text-sm font-semibold text-slate-800">시드 키워드<input value={seedKeyword} onChange={(event) => onSeedKeywordChange(event.target.value)} placeholder="예: 욕실 수납, 주방 정리, 차량용 수납" className="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" /></label><p className="mt-1 text-xs text-slate-600">비워두면 goods_key 기준으로 키워드 엔진이 자동 진행합니다.</p><UploadRowsTable rows={rows} /><div className="mt-5 flex flex-wrap gap-3"><button type="button" onClick={onPreview} disabled={!!busy || blocked} className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white disabled:bg-slate-400">키워드 엔진 입력값 확인</button><button type="button" onClick={onDispatch} disabled={!!busy || blocked} className="rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white disabled:bg-slate-400">키워드 엔진 실행</button><button type="button" onClick={onFetchRuns} disabled={!!busy} className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 disabled:bg-slate-100">키워드 실행 결과 확인</button></div>{preview ? <details className="mt-4 rounded-lg border border-slate-200 p-3"><summary className="cursor-pointer text-sm font-bold text-slate-700">상세 실행 정보 열기</summary><pre className="mt-3 overflow-x-auto rounded-lg bg-slate-950 p-3 text-xs text-slate-50">{JSON.stringify(preview, null, 2)}</pre></details> : null}{dispatchResult ? <dl className="mt-4 grid gap-3 text-sm"><ResultRow label="repo" value={dispatchResult.repo ?? "-"} /><ResultRow label="workflowFile" value={dispatchResult.workflowFile ?? "-"} /><ResultRow label="actionsUrl" value={dispatchResult.actionsUrl ?? "-"} /><ResultRow label="expectedArtifactName" value={dispatchResult.expectedArtifactName ?? "-"} /><ResultRow label="message" value="키워드 엔진 실행을 요청했습니다. 몇 초 뒤 실행 결과 확인을 눌러주세요." /></dl> : null}{runsResult?.message ? <p className="mt-3 text-sm text-slate-600">{runsResult.message}</p> : null}{latestRunWithArtifact ? <p className="mt-3 rounded-lg bg-emerald-50 p-3 text-sm font-semibold text-emerald-800">가져올 결과물이 있는 최신 실행을 우선 표시합니다.</p> : null}<details className="mt-4"><summary className="cursor-pointer text-sm font-bold text-slate-700">이전 실행 기록 보기</summary><div className="mt-4 space-y-3">{runsResult?.runs?.map((run) => { const expectedArtifact = run.artifacts?.find((artifact) => artifact.expected); return <article key={run.id} className="rounded-lg border border-slate-200 p-4 text-sm"><div className="flex flex-wrap gap-3"><span>run id: <strong>{run.id}</strong></span><span>status: {run.status ?? "-"}</span><span>conclusion: {run.conclusion ?? "-"}</span><span>createdAt: {run.createdAt ?? "-"}</span>{run.htmlUrl ? <Link href={run.htmlUrl} target="_blank" rel="noopener noreferrer" className="font-semibold text-blue-700 underline">GitHub Actions 바로가기</Link> : null}</div><p className={expectedArtifact ? "mt-2 font-semibold text-emerald-700" : "mt-2 text-slate-600"}>{expectedArtifact ? `expected artifact exists: ${expectedArtifact.name}` : "expected artifact exists: no"}</p>{expectedArtifact ? <button type="button" onClick={() => onImport(run, expectedArtifact)} disabled={!!busy || expectedArtifact.expired} className="mt-2 rounded-lg bg-emerald-700 px-3 py-2 text-xs font-semibold text-white disabled:bg-slate-300">결과 가져오기 및 검토 시작</button> : null}</article>; })}</div></details>{importMessage ? <p className="mt-4 rounded-lg bg-emerald-50 p-3 text-sm font-semibold text-emerald-800">{importMessage}</p> : null}{importMessage ? <Link href="/keyword-review-queue?from=product-launch-flow" target="_blank" rel="noopener noreferrer" className="mt-3 inline-flex rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700">개별 키워드 검토 화면에서 열기</Link> : null}</section>;
}

function PriceVerificationTable({ rows, showAll, onShowAllChange }: { rows: ProductLaunchPriceVerificationRow[]; showAll: boolean; onShowAllChange: (value: boolean) => void }) {
  return <div className="mt-4"><div className="mb-3 flex flex-wrap gap-2"><button type="button" onClick={() => onShowAllChange(false)} className={showAll ? "rounded-lg border border-slate-300 px-3 py-2 text-xs font-bold text-slate-700" : "rounded-lg bg-slate-900 px-3 py-2 text-xs font-bold text-white"}>문제만 보기</button><button type="button" onClick={() => onShowAllChange(true)} className={showAll ? "rounded-lg bg-slate-900 px-3 py-2 text-xs font-bold text-white" : "rounded-lg border border-slate-300 px-3 py-2 text-xs font-bold text-slate-700"}>전체 보기</button></div><div className="overflow-x-auto"><table className="min-w-full border-collapse text-sm"><thead><tr className="bg-slate-50 text-left"><th className="border border-slate-200 px-3 py-2">goods_key</th><th className="border border-slate-200 px-3 py-2">mall_key</th><th className="border border-slate-200 px-3 py-2">mall_name</th><th className="border border-slate-200 px-3 py-2">planned_sale_price</th><th className="border border-slate-200 px-3 py-2">verified_sale_price</th><th className="border border-slate-200 px-3 py-2">verification_status</th><th className="border border-slate-200 px-3 py-2">message</th></tr></thead><tbody>{rows.length > 0 ? rows.map((row, index) => <tr key={`${row.goods_key}-${row.mall_key}-${index}`}><td className="border border-slate-200 px-3 py-2 font-mono">{row.goods_key ?? "-"}</td><td className="border border-slate-200 px-3 py-2 font-mono">{row.mall_key ?? "-"}</td><td className="border border-slate-200 px-3 py-2">{row.mall_name ?? "-"}</td><td className="border border-slate-200 px-3 py-2">{row.planned_sale_price ?? "-"}</td><td className="border border-slate-200 px-3 py-2">{row.verified_sale_price ?? "-"}</td><td className="border border-slate-200 px-3 py-2">{row.verification_status ?? "-"}</td><td className="border border-slate-200 px-3 py-2">{row.message || getPriceVerificationMessage(row.verification_status)}</td></tr>) : <tr><td className="border border-slate-200 px-3 py-2 text-slate-500" colSpan={7}>표시할 가격 검증 문제가 없습니다.</td></tr>}</tbody></table></div></div>;
}

function FinalChecklist({ priceVerificationFailed, keywordSucceeded }: { priceVerificationFailed: boolean; keywordSucceeded: boolean }) { const items = ["상품업로드 결과 확인", "goods_key 6개 확인", "ptn_goods_cd suffix 기반 상품그룹 인식 확인", priceVerificationFailed ? "가격 검증 미완료" : "가격설정 완료 확인", "상품명/키워드 단계는 MVP 기준 동일 적용 예정", "샵플링 마켓전송은 수동으로 진행"]; return <section className="rounded-2xl border border-amber-200 bg-amber-50 p-6 shadow-sm"><h2 className="text-lg font-bold text-amber-950">최종 체크리스트</h2><ul className="mt-3 list-disc space-y-1 pl-5 text-sm text-amber-900">{items.map((item) => <li key={item}>{item}</li>)}</ul><p className="mt-4 rounded-lg bg-white p-3 text-sm font-bold text-red-700">{priceVerificationFailed && keywordSucceeded ? "상품명/검색어 반영은 완료됐지만, 일부 쇼핑몰 가격이 비어 있어 최종 출시 완료로 볼 수 없습니다." : "마켓전송은 현재 OPS Center에서 자동 실행하지 않습니다. 샵플링 관리자에서 최종 확인 후 직접 전송하세요."}</p></section>; }
...
function isSuccessfulPriceResult(result: PriceActionsResult | null) {
  ...
  return status === "success" && Number(summary?.missing_price_count ?? 0) === 0 && Number(summary?.missing_mall_row_count ?? 0) === 0 && Number(summary?.mismatch_count ?? 0) === 0 && Number(summary?.unverified_count ?? 0) === 0;
}
function hasPriceVerificationIssue(result: PriceActionsResult | null) { const summary = result?.summary; return Number(summary?.missing_price_count ?? 0) > 0 || Number(summary?.missing_mall_row_count ?? 0) > 0 || Number(summary?.mismatch_count ?? 0) > 0 || Number(summary?.unverified_count ?? 0) > 0; }
function isPriceVerificationIssueStatus(status: unknown) { return ["missing_price", "missing_mall_row", "mismatch", "unverified"].includes(String(status ?? "")); }
function getPriceVerificationMessage(status: unknown) {
  if (status === "missing_price") return "샵플링 화면 기준 가격이 비어 있거나 0원입니다.";
  if (status === "missing_mall_row") return "해당 상품에 쇼핑몰 가격 행이 없어 가격을 반영할 수 없습니다.";
  if (status === "mismatch") return "계획 가격과 샵플링 확인 가격이 다릅니다.";
  if (status === "unverified") return "가격 반영 여부를 확인하지 못했습니다.";
  return "-";
}
...
function buildCockpit(state: { hasUploadRequest: boolean; uploadActive: boolean; uploadSuccess: boolean; uploadFailed: boolean; priceActive: boolean; priceSuccess: boolean; priceFailed: boolean; keywordActive: boolean; keywordSuccess: boolean; keywordFailed: boolean }) {
  ...
  const nextAction = primaryAction === "failed" ? (state.priceFailed ? "가격 오류 확인 또는 가격설정 재실행이 필요합니다." : "문제가 발생했습니다. 실패 원인을 확인하세요.") : state.uploadActive ? "상품업로드 결과를 확인하는 중입니다. 잠시만 기다려주세요." : state.priceActive ? "가격설정 결과를 확인하는 중입니다. 잠시만 기다려주세요." : state.keywordActive ? "키워드 결과를 확인하는 중입니다. 잠시만 기다려주세요." : primaryAction === "upload" ? "행 번호를 입력하고 상품업로드를 시작하세요." : primaryAction === "price" ? (state.priceFailed ? "가격 오류 확인 또는 가격설정 재실행이 필요합니다." : "상품업로드가 완료되었습니다. 이제 가격설정을 시작하세요.") : primaryAction === "keyword" ? "가격설정이 완료되었습니다. 이제 키워드 dry_run을 시작하세요." : primaryAction === "review" ? "키워드 결과가 준비되었습니다. 검토 화면에서 확인하세요." : steps.find((step) => step.name === currentStage)?.action ?? "상품업로드 시작";
