...
export function ProductLaunchFlow() {
  ...
  const launchReviewSummary: LaunchReviewSummary = normalizeLaunchReviewSummary(keywordApplyState, isSuccessfulPriceResult(priceActionsResult), goodsKeys.length);
  const agentState = getOperatorAgentState({ uploadRunning: uploadRunning || uploadFetching || uploadPolling, uploadReady: goodsKeys.length > 0, priceRunning: priceRunning || priceFetching || pricePolling, priceReady: isSuccessfulPriceResult(priceActionsResult), keywordRunning: keywordBusy === "dispatch" || keywordBusy === "runs" || keywordPolling || isKeywordRunning(keywordRunsResult), reviewReady: keywordApplyState?.dryRunStatus === "success", reviewBlocked: launchReviewSummary.blockedCount > 0, realApplying: keywordApplyState?.realApplyStatus === "queued" || keywordApplyState?.realApplyStatus === "running", realApplied: keywordRealApplySucceeded, finalPriceRunning: finalPriceActive, finalPriceDone, failed: cockpit.primaryAction === "failed" || hasUploadFailure(uploadActionsResult) || hasPriceFailure(priceActionsResult) || hasKeywordFailure(keywordRunsResult) || finalPriceFailed });
  const primaryButton = getLaunchPrimaryButton({ agentState, rowExpression, uploadBusy: uploadRunning || uploadFetching || uploadPolling, reviewReady: keywordApplyState?.dryRunStatus === "success", canApprove: launchReviewSummary.canApprove, realApplying: keywordApplyState?.realApplyStatus === "queued" || keywordApplyState?.realApplyStatus === "running", finalPriceActive, finalPriceDone });
  const runPrimaryAction = () => {
    if (primaryButton.action === "upload") void runUploadRequest();
    if (primaryButton.action === "review") void openInlineKeywordReview();
    if (primaryButton.action === "approve") document.getElementById("keyword-shopling-apply-section")?.querySelector("button")?.dispatchEvent(new MouseEvent("click", { bubbles: true }));
    if (primaryButton.action === "finalPrice") void runFinalPriceModify();
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <LaunchHero agentState={agentState} nextAction={primaryButton.label} progress={getLaunchProgress(agentState)} issueCount={issueCount} lastCheckedAt={lastCheckedAt} running={uploadRunning || uploadFetching || uploadPolling || priceRunning || priceFetching || pricePolling || keywordBusy === "dispatch" || keywordBusy === "runs" || keywordPolling || finalPriceActive || keywordApplyState?.realApplyStatus === "running"} />
      <RowInputSection rowExpression={rowExpression} onRowExpressionChange={setRowExpression} onStart={() => void runUploadRequest()} disabled={uploadRunning || !rowExpression.trim()} />
      {goodsKeys.length > 0 ? <RowSeedKeywordSection sourceRowGroups={sourceRowGroups} rowExpression={lastStartedRowExpression || rowExpression} seedKeywordsBySourceRow={seedKeywordsBySourceRow} onSeedKeywordChange={(sourceRowId, value) => setSeedKeywordsBySourceRow((current) => ({ ...current, [sourceRowId]: value }))} /> : null}
      {keywordSummary.artifact || embeddedReviewOpen ? <FinalReviewSection summary={launchReviewSummary} rows={buildCompactReviewRows(uploadRows, keywordApplyState)} /> : null}
      {embeddedReviewOpen ? <KeywordReviewWorkspace mode="embedded" launchContext={{ rowExpression: lastStartedRowExpression || rowExpression, uploadRequestId: currentUploadRequestId || uploadRequestId, priceRequestId, keywordRunId: keywordRunsResult?.runs?.[0]?.id, artifactName: keywordSummary.artifact?.name, importedAt: keywordImportedAt || "확인 중", goodsKeyCount: goodsKeys.length, productGroupCount: new Set(uploadRows.map((row) => inferProductGroupFromPtnGoodsCd(row.ptn_goods_cd ?? ""))).size, uploadRows, goodsKeys, goodsKeyGroupMap: buildGoodsKeyGroupMap(uploadRows), manualTitleOverridesByGoodsKey, manualKeywordOverridesByGoodsKey, seedKeywordsByGoodsKey, autoApplyToShopling: false, autoApplyConfirmationText: "" }} onApplyStateChange={setKeywordApplyState} /> : null}
      <ApprovalSection label={primaryButton.label} disabled={primaryButton.disabled} onClick={runPrimaryAction} />
      {finalPriceDone ? <section className="rounded-2xl border border-emerald-200 bg-emerald-50 p-6 text-lg font-black text-emerald-900">출시 완료</section> : null}
      <details className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <summary className="cursor-pointer text-lg font-bold text-slate-950">개발자 진단 보기</summary>
        <button type="button" onClick={resetProductLaunchSession} className="mt-4 rounded-lg border border-red-300 px-4 py-2 text-sm font-bold text-red-700">현재 상품출시 작업 초기화</button>
        <div className="mt-4 grid gap-2 text-xs text-slate-600"><ResultRow label="upload" value={uploadRequestId || "-"} mono /><ResultRow label="price" value={priceRequestId || "-"} mono /><ResultRow label="keyword" value={String(keywordRunsResult?.runs?.[0]?.id ?? "-")} mono /><ResultRow label="final price" value={finalPriceRequestId || "-"} mono /></div>
        <ErrorDrawer title="문제 확인" uploadResult={uploadActionsResult} priceResult={priceActionsResult} keywordResult={keywordRunsResult} requestId={previousRequestId} actionsUrl={keywordGithubActionsUrl ?? priceGithubActionsUrl ?? uploadGithubActionsUrl} />
      </details>
    </div>
  );
}
...
type LaunchReviewSummary = {
  safeCount: number;
  warningCount: number;
  blockedCount: number;
  underfilledSearchKeywordCount: number;
  dangerousKeywordCount: number;
  totalCount: number;
  canApprove: boolean;
  blockReasons: string[];
};

type OperatorAgentState = "시작 전" | "상품업로드 중" | "가격 1차 적용 중" | "행별 키워드 입력 대기" | "AI 검토 생성 중" | "출시 보류 - 승인 대기" | "출시 보류 - 차단 항목 있음" | "실제 반영 중" | "가격 최종 재적용 중" | "출시 완료" | "실패 - 문제 확인 필요";

function LaunchHero({ agentState, nextAction, progress, issueCount, lastCheckedAt, running }: { agentState: OperatorAgentState; nextAction: string; progress: number; issueCount: number; lastCheckedAt: Date | null; running: boolean }) {
  return <section className="rounded-3xl border border-blue-200 bg-gradient-to-br from-blue-50 to-white p-6 shadow-sm">
    <p className="text-sm font-black text-blue-700">AI 상품출시 에이전트</p>
    <div className="mt-3 grid gap-4 md:grid-cols-5"><ResultRow label="현재 단계" value={agentState} /><ResultRow label="다음 작업" value={nextAction} /><ResultRow label="진행률" value={`${progress}%`} /><ResultRow label="문제 수" value={String(issueCount)} /><ResultRow label="마지막 확인 시각" value={lastCheckedAt ? lastCheckedAt.toLocaleTimeString("ko-KR") : "-"} /></div>
    <div className="mt-4 h-2 overflow-hidden rounded-full bg-blue-100"><div className="h-full rounded-full bg-blue-600 transition-all" style={{ width: `${progress}%` }} /></div>
    {running ? <p className="mt-4 inline-flex items-center gap-2 text-sm font-bold text-blue-800"><span className="size-4 animate-spin rounded-full border-2 border-blue-200 border-t-blue-700" />진행 중입니다.</p> : null}
  </section>;
}

function RowInputSection({ rowExpression, onRowExpressionChange, onStart, disabled }: { rowExpression: string; onRowExpressionChange: (value: string) => void; onStart: () => void; disabled: boolean }) {
  return <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><label className="block text-sm font-bold text-slate-800">실재고 시트 행 번호 입력<input value={rowExpression} onChange={(event) => onRowExpressionChange(event.target.value)} placeholder="950 또는 950,951 또는 950-955" className="mt-2 w-full rounded-lg border border-slate-300 px-3 py-3 text-sm" /></label><button type="button" onClick={onStart} disabled={disabled} className="mt-4 rounded-lg bg-blue-700 px-5 py-3 text-sm font-black text-white disabled:bg-slate-300">상품출시 시작</button></section>;
}

function RowSeedKeywordSection({ sourceRowGroups, seedKeywordsBySourceRow, onSeedKeywordChange }: { sourceRowGroups: ReturnType<typeof buildLaunchSourceRowGroups>; rowExpression: string; seedKeywordsBySourceRow: Record<string, string>; onSeedKeywordChange: (sourceRowId: string, value: string) => void; }) {
  return <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><h2 className="text-xl font-black text-slate-950">행별 핵심 키워드</h2><p className="mt-2 whitespace-pre-line text-sm font-semibold text-slate-700">{`실재고 시트 행마다 좋은 키워드를 한 번만 입력하세요.
같은 행에서 생성된 도매/소매 상품들은 이 키워드를 함께 사용합니다.
AI가 이 키워드로 쇼핑몰별 상품명과 상품별 검색어를 자동 생성합니다.`}</p><div className="mt-4 overflow-x-auto"><table className="min-w-full border-collapse text-sm"><thead><tr className="bg-slate-50 text-left"><th className="border border-slate-200 px-3 py-2">실재고 행</th><th className="border border-slate-200 px-3 py-2">생성 상품 요약</th><th className="border border-slate-200 px-3 py-2">현재 상품명</th><th className="border border-slate-200 px-3 py-2">핵심 키워드 입력</th><th className="border border-slate-200 px-3 py-2">상태</th></tr></thead><tbody>{sourceRowGroups.map((group) => { const value = seedKeywordsBySourceRow[group.sourceRowId] ?? ""; const status = group.mappingMissing ? "확인 필요" : value.trim() ? "준비 완료" : group.goodsKeys.length > 0 ? "엔진 보강" : "키워드 입력 권장"; return <tr key={group.sourceRowId}><td className="border border-slate-200 px-3 py-2 font-bold">{group.sourceRowId}</td><td className="border border-slate-200 px-3 py-2">{group.goodsKeys.length}개 상품</td><td className="border border-slate-200 px-3 py-2">{group.currentTitle || "-"}</td><td className="border border-slate-200 px-3 py-2"><input value={value} onChange={(event) => onSeedKeywordChange(group.sourceRowId, event.target.value)} placeholder="게임패드,컨트롤러,조이스틱,미니" className="w-full rounded-lg border border-slate-300 px-3 py-2" /></td><td className="border border-slate-200 px-3 py-2 font-bold">{status}</td></tr>; })}</tbody></table></div></section>;
}

function FinalReviewSection({ summary, rows }: { summary: LaunchReviewSummary; rows: CompactReviewRow[] }) {
  return <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><h2 className="text-xl font-black text-slate-950">최종 반영 전 검토</h2><div className="mt-4 grid gap-3 sm:grid-cols-5"><SummaryCard label="안전" value={summary.safeCount} /><SummaryCard label="검토 필요" value={summary.warningCount} /><SummaryCard label="차단" value={summary.blockedCount} /><SummaryCard label="검색어 부족" value={summary.underfilledSearchKeywordCount} /><SummaryCard label="위험 키워드" value={summary.dangerousKeywordCount} /></div>{summary.blockedCount > 0 ? <p className="mt-3 rounded-lg border border-red-200 bg-red-50 p-3 text-sm font-bold text-red-800">차단 항목이 있어 실제 반영할 수 없습니다.</p> : summary.warningCount > 0 ? <p className="mt-3 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm font-bold text-amber-900">검토 필요 항목이 있지만 반영은 가능합니다. 필요하면 전체 항목을 확인하세요.</p> : null}<details className="mt-4"><summary className="cursor-pointer font-bold text-slate-800">전체 항목 펼쳐보기</summary><div className="mt-3 overflow-x-auto"><table className="min-w-full border-collapse text-sm"><thead><tr className="bg-slate-50 text-left"><th className="border border-slate-200 px-3 py-2">실재고 행</th><th className="border border-slate-200 px-3 py-2">상품그룹</th><th className="border border-slate-200 px-3 py-2">쇼핑몰</th><th className="border border-slate-200 px-3 py-2">생성 상품명</th><th className="border border-slate-200 px-3 py-2">검색어</th><th className="border border-slate-200 px-3 py-2">상태</th><th className="border border-slate-200 px-3 py-2">사유</th></tr></thead><tbody>{rows.map((row) => <tr key={row.id}><td className="border border-slate-200 px-3 py-2">{row.sourceRow}</td><td className="border border-slate-200 px-3 py-2">{row.productGroup}</td><td className="border border-slate-200 px-3 py-2">{row.mall}</td><td className="border border-slate-200 px-3 py-2">{row.title}</td><td className="border border-slate-200 px-3 py-2">{row.searchKeywords}</td><td className="border border-slate-200 px-3 py-2">{row.status}</td><td className="border border-slate-200 px-3 py-2">{row.reason}</td></tr>)}</tbody></table></div></details></section>;
}

function ApprovalSection({ label, disabled, onClick }: { label: string; disabled: boolean; onClick: () => void }) { return <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><button type="button" onClick={onClick} disabled={disabled} className="w-full rounded-xl bg-red-600 px-5 py-4 text-base font-black text-white disabled:bg-slate-300">{label}</button></section>; }

type CompactReviewRow = { id: string; sourceRow: string; productGroup: string; mall: string; title: string; searchKeywords: string; status: string; reason: string };
function buildCompactReviewRows(rows: ProductLaunchUploadRow[], state: KeywordApplyState | null): CompactReviewRow[] { return rows.slice(0, 50).map((row, index) => ({ id: `${row.goods_key ?? index}`, sourceRow: String(row.row ?? "-"), productGroup: inferProductGroupFromPtnGoodsCd(row.ptn_goods_cd ?? "").productGroup, mall: String(row.channel ?? "전체"), title: String(row.product_name ?? row.title ?? row.productTitle ?? "검토 생성 후 확인"), searchKeywords: state?.dryRunStatus === "success" ? "생성 완료" : "검토 생성 중", status: state?.dryRunStatus === "success" ? "안전" : "확인 중", reason: state?.dryRunStatus === "success" ? "반영 가능" : "검토 생성 대기" })); }
function normalizeLaunchReviewSummary(state: KeywordApplyState | null, priceReady: boolean, goodsKeyCount: number): LaunchReviewSummary { const blockedCount = Number(state?.blankMallTitleBlockedCount ?? state?.failedCount ?? 0); const warningCount = Number(state?.warningCount ?? 0); const safeCount = state?.dryRunStatus === "success" ? Math.max(0, goodsKeyCount - blockedCount) : 0; const totalCount = Math.max(goodsKeyCount, safeCount + blockedCount); return { safeCount, warningCount, blockedCount, underfilledSearchKeywordCount: warningCount, dangerousKeywordCount: blockedCount, totalCount, canApprove: priceReady && state?.dryRunStatus === "success" && blockedCount === 0 && state.realApplyStatus !== "running" && state.realApplyStatus !== "queued", blockReasons: blockedCount > 0 ? ["차단 항목이 있어 실제 반영할 수 없습니다."] : [] }; }
function getOperatorAgentState(input: { uploadRunning: boolean; uploadReady: boolean; priceRunning: boolean; priceReady: boolean; keywordRunning: boolean; reviewReady: boolean; reviewBlocked: boolean; realApplying: boolean; realApplied: boolean; finalPriceRunning: boolean; finalPriceDone: boolean; failed: boolean }): OperatorAgentState { if (input.failed) return "실패 - 문제 확인 필요"; if (input.finalPriceDone) return "출시 완료"; if (input.finalPriceRunning) return "가격 최종 재적용 중"; if (input.realApplying) return "실제 반영 중"; if (input.reviewBlocked) return "출시 보류 - 차단 항목 있음"; if (input.reviewReady) return "출시 보류 - 승인 대기"; if (input.keywordRunning || input.priceReady) return "AI 검토 생성 중"; if (input.uploadReady && !input.priceReady) return input.priceRunning ? "가격 1차 적용 중" : "행별 키워드 입력 대기"; if (input.uploadRunning) return "상품업로드 중"; return "시작 전"; }
function getLaunchProgress(state: OperatorAgentState) { return ({ "시작 전": 0, "상품업로드 중": 15, "가격 1차 적용 중": 30, "행별 키워드 입력 대기": 45, "AI 검토 생성 중": 65, "출시 보류 - 승인 대기": 80, "출시 보류 - 차단 항목 있음": 80, "실제 반영 중": 88, "가격 최종 재적용 중": 95, "출시 완료": 100, "실패 - 문제 확인 필요": 100 } as Record<OperatorAgentState, number>)[state]; }
function getLaunchPrimaryButton(input: { agentState: OperatorAgentState; rowExpression: string; uploadBusy: boolean; reviewReady: boolean; canApprove: boolean; realApplying: boolean; finalPriceActive: boolean; finalPriceDone: boolean }) { if (input.finalPriceDone) return { label: "출시 완료 확인", action: "none", disabled: true }; if (input.finalPriceActive) return { label: "가격 최종 재적용 중", action: "none", disabled: true }; if (input.realApplying) return { label: "실제 반영 확인 중", action: "none", disabled: true }; if (input.reviewReady || input.agentState === "출시 보류 - 차단 항목 있음") return { label: "승인하고 실제 반영 실행", action: "approve", disabled: !input.canApprove }; if (input.agentState === "AI 검토 생성 중") return { label: "검토 결과 확인 중", action: "none", disabled: true }; if (input.agentState === "행별 키워드 입력 대기") return { label: "키워드 입력 후 검토 생성", action: "review", disabled: false }; return { label: "상품출시 시작", action: "upload", disabled: input.uploadBusy || !input.rowExpression.trim() }; }
...
function AILaunchAgentBoard({ state, productCount, mallCount, titleTargetCount, keywordWarningCount, issueCount, actualApplyDone, keywordApplyState, priceIssueState, onNext, initialPriceRequestId, finalPriceRequestId, finalPriceActionsResult, finalPriceActive, finalPriceDone, finalPriceFailed, finalPriceTargetCount }: { state: ReturnType<typeof buildCockpit>; productCount: number; mallCount: number; titleTargetCount: number; keywordWarningCount: number; issueCount: number; actualApplyDone: boolean; keywordApplyState: KeywordApplyState | null; priceIssueState: PriceIssueState; onNext: () => void; initialPriceRequestId: string; finalPriceRequestId: string; finalPriceActionsResult: PriceActionsResult | null; finalPriceActive: boolean; finalPriceDone: boolean; finalPriceFailed: boolean; finalPriceTargetCount: number }) {
  ...
  const finalVerdict = blockedByApply ? "출시 보류 - 차단 항목 있음" : realApplyRunning ? "샵플링 실제 반영 중" : finalPriceActive ? "가격 최종 재적용 중" : finalPriceFailed ? "출시 보류 - 가격 최종 재적용 실패" : keywordRealApplySucceeded && !finalPriceDone ? "가격 최종 재적용 중" : actualApplyDone ? "출시 완료" : dryRunComplete ? "출시 보류 - 승인 대기" : uploadAndPriceComplete ? "키워드 검토 준비 중" : hasCriticalPriceIssue ? "출시 보류 - 가격 확인 필요" : state.primaryAction === "wait" ? "진행 중" : "검토 준비";
