...
function KeywordShoplingApplySection({
  ...
  const blockedCount = preflightResult?.summary.blockedCount ?? 0;
  const safeCount = preflightResult?.summary.eligibleCount ?? 0;
  ...
  return (
    <section id="keyword-shopling-apply-section" className="mt-6 rounded-xl border border-slate-200 bg-white shadow-sm">
      <div className="border-b border-slate-200 p-4 sm:p-5">
        <h2 className="font-semibold text-slate-950">최종 반영 전 검토</h2>
        <p className="mt-2 text-sm text-slate-600">검색어는 상품별 1세트로 반영됩니다. 쇼핑몰별 차별화는 상품명에서 적용합니다.</p>
        <div className="mt-4 grid gap-3 sm:grid-cols-5">
          <SummaryCard label="안전" value={safeCount} />
          <SummaryCard label="검토 필요" value={warningCount} />
          <SummaryCard label="차단" value={blockedCount} />
