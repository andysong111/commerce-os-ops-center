...
type ActiveBatch = { mode: "PARALLEL" | "SINGLE"; batchId: string; jobs: SyncJob[]; terminalJobIds: Set<string> };
type LocalException = { jobId: string; barcode: string; outcome: Exclude<SyncOutcome, "STARTED">; message: string };
...
function statusLabel(value: DesiredStatus) { return value === "SOLD_OUT" ? "품절" : "판매중"; }
function kindLabel(value: ProductKind) { return value === "OPTION" ? "옵션" : "단품"; }
...
function stableEventId(prefix: string, job: SyncJob, outcome: SyncOutcome, finishedAt?: number) {
  return [prefix, job.jobId, outcome, Number(finishedAt || 0)].filter(Boolean).join(":").slice(0, 480);
}
function prioritySort(left: SyncJob, right: SyncJob) {
  const difference = (left.desiredStatus === "SOLD_OUT" ? 0 : 1) - (right.desiredStatus === "SOLD_OUT" ? 0 : 1);
...
export function StockSyncOperationalQueuePanel() {
  ...
  const loadQueue = useCallback(async (fresh = true) => {
    try {
      ...
      if (connectionFailed.current) setNotice("재고 조회 연결이 복구되었습니다. 실제 송신은 기존 승인 절차를 유지합니다.");
      ...
      setJobs([]);
      setReport(null);
      setNotice(error instanceof Error ? error.message : "운영 재고상태 큐 조회 실패");
      throw error;
    }
  }, []);

  const recordSync = useCallback(async (job: SyncJob, outcome: SyncOutcome, message: string, evidence?: unknown, finishedAt?: number) => {
    // Never route writes through the read retry/coalescing helper.
    const response = await fetch("/api/inventory-stock-control/sync", {
      method: "POST",
      headers: { "content-type": "application/json", accept: "application/json" },
      body: JSON.stringify({
        eventId: outcome === "STARTED" ? stableEventId("shopling-stock-started", job, outcome) : stableEventId("shopling-stock-result", job, outcome, finishedAt || Date.now()),
  ...
  const launchNext = useCallback(async () => {
    if (!continueAfterBatch.current) {
      setDraining(false); setPhase("PAUSED"); activeBatch.current = null; setRunningBarcodes([]); return;
    }
    const payload = await loadQueue();
    const available = payload.jobs.filter((job) => !excludedThisRun.current.has(job.jobId)).sort(prioritySort);
    if (!available.length) {
      activeBatch.current = null; setRunningBarcodes([]); setDraining(false); setPhase("DONE");
      setNotice(localExceptions.length || blockedRows.length
  ...
  useEffect(() => {
    ...
    const polling = startInventoryPolling({
      task: () => loadQueue(false),
      active: () => !activeBatch.current && document.visibilityState === "visible" && navigator.onLine !== false && (!details || details.open),
      onError: (error) => setNotice(error instanceof Error ? error.message : "운영 큐 조회 실패"),
    });
    ...
    return () => {
      polling.stop(); details?.removeEventListener("toggle", wake);
      document.removeEventListener("visibilitychange", wake); window.removeEventListener("online", wake);
    };
  }, [loadQueue]);

  useEffect(() => {
    const queueReadFailed = (error: unknown) => {
      continueAfterBatch.current = false; setDraining(false); setPhase("QUEUE_READ_FAILED");
      setNotice(error instanceof Error ? error.message : "연결 복구 전까지 다음 배정을 중지합니다.");
    };
    const onMessage = (event: MessageEvent) => {
      ...
      if (type === PARALLEL_STATUS) {
        ...
        const nextPhase = String(data.phase || ""); if (nextPhase) setPhase(nextPhase);
        if (data.ok === false) {
          void (async () => {
            ...
            for (const job of batch.jobs) {
              ...
              await recordSync(job, "FAILED", message, { code: String(data.code || "OPERATIONAL_PARALLEL_START_FAILED"), operationalQueue: true, operationalBatchId: batch.batchId }).catch(() => undefined);
              setLocalExceptions((current) => [...current.filter((row) => row.jobId !== job.jobId), { jobId: job.jobId, barcode: job.barcode, outcome: "FAILED", message }]);
            }
            activeBatch.current = null; setRunningBarcodes([]);
            setNotice(`${message} · 해당 2건은 이번 실행에서 자동 재시도하지 않습니다.`);
            window.setTimeout(() => { void launchNext().catch(queueReadFailed); }, 800);
          })();
        }
        return;
      }
      ...
      void (async () => {
        try {
          ...
          if (result.outcome !== "SUCCEEDED") {
            excludedThisRun.current.add(job.jobId);
            const exceptionOutcome: Exclude<SyncOutcome, "STARTED"> = result.outcome;
            setLocalExceptions((current) => [...current.filter((row) => row.jobId !== job.jobId), { jobId: job.jobId, barcode: job.barcode, outcome: exceptionOutcome, message: result.message || "원인 확인 필요" }]);
          }
          if (batch.terminalJobIds.size < batch.jobs.length) {
            setNotice(`${job.barcode} ${result.outcome} · 같은 배치의 다른 Lane 결과를 기다리며 창 닫힘 여부는 무시합니다.`); return;
          }
          activeBatch.current = null; setRunningBarcodes([]);
          await loadQueue();
          setNotice(batch.jobs.every((row) => !excludedThisRun.current.has(row.jobId))
            ? "현재 운영 배치 완료 · 다음 대기건을 자동으로 이어서 배정합니다."
            : "현재 운영 배치 종료 · 실패/불확실 건은 예외 큐로 격리하고 정상 대기건만 계속 처리합니다.");
          window.setTimeout(() => { void launchNext().catch(queueReadFailed); }, 900);
        } catch (error) {
          handledResults.current.delete(resultKey); continueAfterBatch.current = false;
          activeBatch.current = null; setRunningBarcodes([]); setDraining(false); setPhase("RESULT_PERSIST_FAILED");
          setNotice(error instanceof Error ? error.message : "운영 큐 결과 저장 실패");
        }
      })();
    };
    window.addEventListener("message", onMessage);
    window.postMessage({ type: "COMMERCE_OS_SHOPLING_STOCK_SYNC_EXTENSION_PING" }, window.location.origin);
  ...
  const startDrain = async () => {
    ...
    continueAfterBatch.current = true; setDraining(true); setPhase("QUEUE_DRAIN_START");
    try { await launchNext(); }
    catch (error) { continueAfterBatch.current = false; setDraining(false); setPhase("START_FAILED"); setNotice(error instanceof Error ? error.message : "운영 큐 시작 실패"); }
  };
  const pauseAfterCurrent = () => {
    continueAfterBatch.current = false;
    setNotice(activeBatch.current ? "현재 실행 중인 Lane은 끝까지 기록하고, 그 다음 대기건부터 자동배정을 중지합니다." : "운영 큐 자동배정을 중지했습니다.");
    if (!activeBatch.current) { setDraining(false); setPhase("PAUSED"); }
  };
  return (
    <section ref={panelRef} className="rounded-2xl border border-sky-200 bg-sky-50 p-5 shadow-sm">
      ...
      {notice ? <div role="status" className="mt-4 rounded-xl border border-sky-200 bg-white px-4 py-3 text-sm font-bold text-slate-800">{notice}</div> : null}
      <div className="mt-4 grid gap-3 sm:grid-cols-4">
        <div className="rounded-xl border border-sky-200 bg-white p-4"><div className="text-xs font-bold text-slate-500">실행 가능 대기</div><div className="mt-1 text-2xl font-black text-slate-950">{report ? jobs.length : "—"}</div></div>
        <div className="rounded-xl border border-sky-200 bg-white p-4"><div className="text-xs font-bold text-slate-500">현재 Lane</div><div className="mt-1 text-lg font-black text-slate-950">{runningBarcodes.length ? runningBarcodes.join(" + ") : "없음"}</div></div>
        <div className="rounded-xl border border-sky-200 bg-white p-4"><div className="text-xs font-bold text-slate-500">차단/확인 필요</div><div className="mt-1 text-2xl font-black text-slate-950">{report ? blockedRows.length + localExceptions.length : "—"}</div></div>
      ...
      <div className="mt-4 overflow-x-auto rounded-xl border border-sky-200 bg-white">
        <table className="min-w-[900px] text-left text-xs">
          <thead className="bg-sky-100/70 text-slate-600"><tr><th className="px-3 py-3">우선순위 / B코드</th><th className="px-3 py-3">형태</th><th className="px-3 py-3 text-right">정확재고</th><th className="px-3 py-3">목표상태</th><th className="px-3 py-3">상태전환 시점</th></tr></thead>
      ...
      {(blockedRows.length > 0 || localExceptions.length > 0) ? <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4">
        <h3 className="text-sm font-black text-amber-950">예외 큐 · 자동 재시도 금지</h3>
        <div className="mt-2 space-y-2 text-xs text-amber-950">
          {blockedRows.map((row) => <div key={`blocked:${row.barcode}`} className="rounded-lg bg-white px-3 py-2"><strong>{row.barcode}</strong> · {statusLabel(row.desiredStatus)} · {row.syncBlockReason || "수동 확인 필요"}</div>)}
          {localExceptions.map((row) => <div key={`local:${row.jobId}`} className="rounded-lg bg-white px-3 py-2"><strong>{row.barcode}</strong> · {row.outcome} · {row.message}</div>)}
        </div>
      </div> : null}
      <p className="mt-3 text-xs leading-5 text-slate-500">화면과 이 실행 영역이 보일 때만 큐를 자동 조회합니다. 정상 상태는 30초 간격이며, 실패하면 최대 3분까지 간격을 늘립니다. 실제 송신은 위 버튼으로 승인하며 쓰기 요청은 자동 재시도하지 않습니다.</p>
    </section>
  );
}
