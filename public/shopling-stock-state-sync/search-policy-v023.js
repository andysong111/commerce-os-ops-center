...
(() => {
  ...
  function datePair(form) {
    const inputs = [...form.querySelectorAll("input")].filter((el) => {
      ...
      return /^20\d{6}$|^201\d{5}$/.test(digits(el.value)) && /일자|날짜|기간|등록일|date/i.test(`${context} ${el.name || ""} ${el.id || ""}`);
    });
    const pairs = [];
  ...
  function applyPeriod(form, setInput, now = new Date()) {
    ...
    for (const [index, value] of [START, end].entries()) if (digits(pair[index].value) !== value) setInput(pair[index], dateValue(pair[index], value));
    const evidence = { start: digits(pair[0].value), end: digits(pair[1].value), expectedStart: START, expectedEnd: end };
    if (evidence.start !== START || evidence.end !== end) return { ok: false, code: "SEARCH_DATE_VERIFY_FAILED", message: "2013-09-12~오늘 최대 검색기간 설정값 검증에 실패했습니다.", evidence };
  ...
  async function awaitRows(token, api, timeoutMs) {
    return api.waitFor(() => { const found = api.rows(token); return found.length ? found : null; }, timeoutMs, 200);
  }
  function bindingEvidence(fieldLabel, token, field, input) {
    return { expectedFieldLabel: fieldLabel, selectedFieldLabel: norm(field?.options?.[field.selectedIndex]?.textContent), expectedToken: norm(token).toUpperCase(), inputToken: norm(input?.value).toUpperCase() };
  }
  function bindingMatches(evidence) { return evidence.selectedFieldLabel === evidence.expectedFieldLabel && evidence.inputToken === evidence.expectedToken; }
  async function search(fieldLabel, token, api) {
    ...
    let ticket = null;
    try { ticket = JSON.parse(sessionStorage.getItem(key) || "null"); } catch {}
    ...
    const resumed = Boolean(ticket && Date.now() - Number(ticket.at || 0) < 90_000 && ticket.fieldLabel === fieldLabel && String(ticket.token || "").toUpperCase() === norm(token).toUpperCase() && queryMatches);
    if (resumed || (completed.has(scope) && queryMatches)) {
      ...
      if (!rows?.length) {
        ...
        return { ok: false, code, message: Number(evidence.totalResultCount || 0) > 0 ? `${token} 조회결과 ${evidence.totalResultCount}건은 확인했지만 정확 행을 worker가 연결하지 못했습니다.` : `${token} 정확 일치 검색결과가 없습니다. 동일 작업에서 검색 버튼을 다시 누르지 않았습니다.`, evidence };
      }
    ...
    if (!input) return { ok: false, code: "SEARCH_INPUT_NOT_BOUND", message: `${fieldLabel}과 같은 검색행의 입력칸을 찾지 못해 검색을 차단했습니다.` };
    if (!api.setInput(input, token)) return { ok: false, code: "SEARCH_INPUT_SET_FAILED", message: `${fieldLabel} 검색어 ${token}을 입력하지 못했습니다.` };
    field = api.getField(fieldLabel) || field; input = api.findInput(field) || input;
    const verifiedBinding = bindingEvidence(fieldLabel, token, field, input);
    if (!bindingMatches(verifiedBinding)) return { ok: false, code: "SEARCH_BINDING_MISMATCH", message: `${fieldLabel} 검색항목과 검색어 입력칸의 결합 검증에 실패해 검색 버튼을 누르지 않았습니다.`, evidence: verifiedBinding };
    ...
    if (!bindingMatches(finalBinding)) return { ok: false, code: "SEARCH_BINDING_MISMATCH", message: "검색 직전 검색항목 또는 검색어가 변경되어 실행을 차단했습니다.", evidence: finalBinding };
    if (!livePair || digits(livePair[0].value) !== START || digits(livePair[1].value) !== todayKst()) return { ok: false, code: "SEARCH_DATE_VERIFY_FAILED", message: "검색 직전 날짜가 변경되어 실행을 차단했습니다." };
    try { sessionStorage.setItem(key, JSON.stringify({ at: Date.now(), documentToken, fieldLabel, token: norm(token).toUpperCase(), period: period.evidence, submitted: true })); }
    ...
    if (!rows?.length) {
      ...
      const code = Number(evidence.totalResultCount || 0) > 0 ? "EXACT_RESULT_ROW_NOT_BOUND" : "EXACT_RESULT_NOT_FOUND";
      return { ok: false, code, message: Number(evidence.totalResultCount || 0) > 0 ? `${token} 조회결과 ${evidence.totalResultCount}건은 확인했지만 정확 행을 worker가 연결하지 못했습니다.` : `${token} 정확 일치 검색결과가 없습니다. 동일 작업에서 검색 버튼을 다시 누르지 않습니다.`, evidence };
    }
    completed.set(scope, true);
