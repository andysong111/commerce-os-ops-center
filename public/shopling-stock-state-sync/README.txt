Commerce OS · Shopling Stock State Sync v0.5.5 HF2

사용자 준비
- 이전 Stock State Sync 확장은 제거/비활성화하고 현재 v0.5.5 HF2 패키지 하나만 사용합니다.
...
- `상품 수정전송`은 과거 실제 옵션전송에 성공한 경로를 유지합니다. HF2는 Shopling의 레거시 이미지형 버튼에서 `alt/title` 텍스트를 읽어 실제 클릭 가능한 부모 요소에 `상품 수정전송` 라벨을 노출한 뒤 기존 직접 click() 경로를 사용합니다.
...
A21 안전장치
- 팝업은 가격조정 확장의 검증된 price-core를 그대로 사용합니다.
