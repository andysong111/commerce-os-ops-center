...
async function runReliableAiCategoryAssignment(button) {
  ...
  setRunStatus("running", `1/4 · ${selected.length}건의 모델명·옵션을 OpenAI가 분석하고 실제 샵플링 카테고리 후보를 좁히고 있습니다.`);
  ...
  try {
    ...
    if (retryItems.length) {
      ...
      setRunStatus(
        "running",
        `3/4 · ${savedCount}건은 보존했습니다. AI 분석에 실패한 ${retryItems.length}건만 자동 재시도하고 있습니다.`,
      );
      try {
