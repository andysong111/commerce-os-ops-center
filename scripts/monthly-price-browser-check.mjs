...
await page.addInitScript(()=>{
  ...
  window.addEventListener('message',e=>{
    ...
    if(m.command==='START'&&window.bridgeHistoryMissingItemId&&m.payload?.itemId===window.bridgeHistoryMissingItemId){
      window.postMessage({channel:m.channel,direction:'response',requestId:m.requestId,response:{ok:false,version:'0.5.10',error:'MONTHLY_PRICE_TRANSMISSION_HISTORY_MISSING'}},location.origin);
      return;
    }
    ...
    if(m.command==='BATCH_START'||m.command==='BATCH_STATUS'){
      ...
      window.postMessage({channel:m.channel,direction:'response',requestId:m.requestId,response:{ok:true,version:'0.5.10',report}},location.origin);
      return;
    }
    const response={ok:true,version:'0.5.10',observation:{fakeReadOnlyFixture:true},report:{token:m.payload.token,fingerprint:m.payload.fingerprint,goodsKey:'1234567',state:'SUCCEEDED',priceOnly:false,priceAndOption:true,saleStatusActivated:true,saleStatusRestored:true}};
