...
const server=createServer(async(req,res)=>{
  ...
  if(req.url.startsWith('/api/china-order-manager/monthly-price')){
    ...
    if(p.action==='start')return res.end(JSON.stringify(snapshot()));
...
try{
  await page.goto(url);await page.getByRole('button',{name:/가격조정 실행/}).click();
  ...
  let bridge=await page.evaluate(()=>window.bridgeEvents);assert.equal(bridge.filter(x=>x.command==='START').length,1);assert.equal(bridge.find(x=>x.command==='START').payload.runId,runId);assert.equal(bridge.find(x=>x.command==='START').payload.newClaim,true);
  await page.getByText('상품별 결과·제외 사유').click();await page.screenshot({path:path.join(out,'one-click-happy-path.png'),fullPage:true});
  events=[];await page.getByRole('button',{name:/가격조정 실행/}).click();await page.waitForTimeout(100);assert.deepEqual(events,['start']);
  scenario='blocked';item=makeItem();events=[];await page.goto(url);await page.getByRole('button',{name:/가격조정 실행/}).click();await page.waitForTimeout(100);assert.deepEqual(events,['start']);assert.equal((await page.evaluate(()=>window.bridgeEvents)).filter(x=>x.command==='START').length,0);
  ...
  scenario='resume';item=makeItem();events=[];await page.goto(url);await page.getByRole('button',{name:/가격조정 실행/}).click();await page.getByText('전송 종료 · 마켓 확인 대기',{exact:true}).waitFor({state:'attached'});
